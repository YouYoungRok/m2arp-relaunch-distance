...
run_experiment.py
------------------
전체 파이프라인을 한번에 실행한다. Lee & Park (2023) 4.4절의
'시뮬레이션 기반 벤치마크(hit ratio@m)'와 동일한 방법론을, 공개되지 않은
Tsinghua 실사용 데이터셋 대신 합성 트레이스로 재현한다.

1. 합성 트레이스 생성 (train 구간 70% / eval 구간 30%로 시간순 분할 — 미래 데이터 누수 방지)
2. train 구간으로 NumPy LSTM 재실행 거리 예측기 학습 (Eq.4 SMSE 손실)
3. eval 구간에서 여러 캐시 용량 m (논문 Table 6과 동일하게 5~15) 에 대해
   LRU / Optimal / M2ARP 세 정책을 Algorithm 1 엔진으로 시뮬레이션
4. hit ratio@m 표 + 그래프를 논문 Table 6 / Fig. 4 형식으로 저장
5. (확장) 앱별 cold/warm-start 비용을 도입한 평균 실행시간(ms) 지표도 함께 산출
   — 이는 원 논문 4.4절에는 없는, 본 프로젝트가 추가한 분석입니다.
"""
...
CACHE_SIZES = list(range(5, 16))     # 논문 Table 6과 동일한 범위 (m = 5..15)
SEED = 0
...
def main():
    ...
    print("=" * 60)
    print("0) 검증: 예측이 전부 0이면 M2ARP == LRU (논문 4.4절 주장)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("2) train 구간으로 재실행 거리 예측기(LSTM) 학습 (SMSE, Eq.4)")
    print("=" * 60)
    ...
    baseline_val_smse, _ = NumpyLSTMRegressor.smse_loss(mean_pred_val, yval, mval)

    model = NumpyLSTMRegressor(cfg.n_apps, EMBED_DIM, HIDDEN_DIM, seed=SEED)
    rng = np.random.default_rng(SEED)
    best_val, best_params, best_epoch = float("inf"), None, 0
    history = []
    for ep in range(1, EPOCHS + 1):
        losses = []
        for xb, yb, mb in tp.iterate_minibatches(Xtr, ytr, mtr, 64, rng):
            if mb.sum() == 0:
                continue
            losses.append(model.train_step(xb, yb, mb, lr=LR))
        val_loss, val_mae = tp.evaluate(model, Xval, yval, mval)
    ...
    model.save(f"{out_dir}/predictor.npz")
    pd.DataFrame(history).to_csv(f"{out_dir}/training_history.csv", index=False)

    print("\n" + "=" * 60)
    print("3) 앱별 학습 데이터 충분성 판정 (Algorithm 1의 A* 집합 / LRU Fallback)")
    print("=" * 60)
    ...
    print(f"학습 데이터 부족(< {LOW_DATA_THRESHOLD}회)으로 LRU-fallback 처리되는 앱: "
          f"{len(low_data_apps)}/{cfg.n_apps}개")

    print("\n" + "=" * 60)
    print(f"4) eval 구간에서 캐시 용량 m={CACHE_SIZES[0]}..{CACHE_SIZES[-1]}에 대해 "
          f"LRU vs Optimal vs M2ARP 시뮬레이션 (Algorithm 1 엔진)")
    print("=" * 60)
    ...
    # ---- 논문 Table 6과 동일한 형식의 hit ratio@m 표 ----
    ...
    hit_table.to_csv(f"{out_dir}/hit_ratio_at_m_table.csv")
    print("\n[논문 Table 6 형식] Hit ratio@m (%):")
    ...
    time_table.to_csv(f"{out_dir}/launch_time_at_m_table.csv")
    print("\n[확장 지표] 평균 실행시간@m (ms):")
    ...
    ax.set_title("App Launch Hit Ratio @ m  (cf. Lee & Park 2023, Fig. 4)")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(f"{plot_dir}/hit_ratio_at_m.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Average App Launch Time @ m  (this project's extension metric)")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(f"{plot_dir}/launch_time_at_m.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("SMSE (Eq. 4)")
    ax.set_title("Relaunch-Distance Predictor Training Curve")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(f"{plot_dir}/training_curve.png", dpi=150)
    plt.close(fig)

    with open(f"{out_dir}/run_config.json", "w") as f:
        json.dump({
            "window": WINDOW, "embed_dim": EMBED_DIM, "hidden_dim": HIDDEN_DIM,
            "epochs": EPOCHS, "lr": LR, "n_days": N_DAYS, "n_apps": cfg.n_apps,
            "max_distance": MAX_DIST, "train_frac": TRAIN_FRAC,
            "low_data_threshold": LOW_DATA_THRESHOLD, "cache_sizes": CACHE_SIZES,
            "seed": SEED, "trace_length": int(T),
            "best_epoch": best_epoch, "best_val_smse": float(best_val),
            "baseline_val_smse": float(baseline_val_smse),
            "lru_equivalence_check": ok,
        }, f, ensure_ascii=False, indent=2)

    print("\n완료. results/ 디렉터리에 CSV 및 그래프가 저장되었습니다.")
